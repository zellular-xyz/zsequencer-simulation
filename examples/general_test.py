...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description='Run the simple app with a specified node URL')
    parser.add_argument('--node-url',
                        type=str,
                        default='http://localhost:6002',
                        help='URL of the node to connect to')
    ...
    sender_thread: threading.Thread = threading.Thread(
        target=send_batches_in_bulk_mode,
        args=[node_url, app_name, TOTAL_BATCHES_COUNT, BULK_COUNT]
    )
